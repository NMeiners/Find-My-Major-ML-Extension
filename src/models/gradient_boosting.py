...
Path: src/models/gradient_boosting.py

Purpose:
  Implements a Gradient Boosting classifier for RIASEC-based career category prediction.
  An intermediate-complexity model that builds an ensemble of decision trees sequentially,
  each correcting the errors of the previous. Uses predict_proba to select top categories,
  then ranks specific jobs via cosine similarity.
...
Assumptions & Constraints:
  ...
  - parameters dict may include any valid sklearn GradientBoostingClassifier kwargs
...
class GradientBoostingModel(BaseModel):
    ...
    Purpose:
      Wraps sklearn's GradientBoostingClassifier to predict career category
      probabilities from RIASEC feature vectors. An intermediate-complexity model
      that tends to outperform logistic regression while remaining interpretable.
    ...
    Inputs:
      ...
      - parameters: dict — sklearn GradientBoostingClassifier kwargs (e.g. n_estimators, learning_rate)
    ...
    def train(self, X_train: pd.DataFrame, y_train: pd.Series) -> None:
        ...
        """
        self._model = GradientBoostingClassifier(**self.parameters)
